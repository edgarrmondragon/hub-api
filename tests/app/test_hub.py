...
async def test_get_plugin_details_meltano_version(
    db: AsyncSession,
    meltano_version: Version | None,
    kinds: dict[str, str],
) -> None:
    """Test get_plugin_details."""
    hub = client.MeltanoHub(db=db)
    details = await hub.get_plugin_details(
        variant_id=ids.VariantID.from_params(plugin_type="extractors", plugin_name="tap-mock", plugin_variant="singer"),
        meltano_version=meltano_version,
    )
    settings = {s.root.name: s.root for s in details.settings}
    assert all(settings[name].kind == kind for name, kind in kinds.items())
