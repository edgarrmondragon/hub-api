...
async def test_find_plugin_name_not_found(hub: client.MeltanoHub) -> None:
    with pytest.raises(client.PluginNotFoundError, match=r"Plugin 'tap-unknown' not found"):
        await hub.find_plugin(plugin_name="tap-unknown")


@pytest.mark.asyncio
async def test_find_plugin_of_type_not_found(hub: client.MeltanoHub) -> None:
    with pytest.raises(client.PluginNotFoundError, match=r"No plugin 'tap-github' found in loaders"):
...
async def test_find_plugin_variant_not_found(hub: client.MeltanoHub) -> None:
    with pytest.raises(client.PluginNotFoundError, match=r"Variant 'acme' of 'tap-github' not found"):
...
@pytest.mark.asyncio
@pytest.mark.xfail(reason="Will raise an error in the future due to ambiguous results", strict=True)
async def test_find_plugin_ambiguous(hub: client.MeltanoHub) -> None:
    with pytest.raises(Exception, match="Some future error message"):
